...
export class DataAggregator {
  ...
  AppTimePerApp: { [key: string]: number }
  lastMousePosition: { x: number; y: number } | null
  lastApplication: any
  lastAppTime: number | null
  ...
  addMouseMove(event) {
    if (this.lastMousePosition !== null) {
      const distance = Math.sqrt(
        (this.lastMousePosition.x - event.x) ** 2 + (this.lastMousePosition.y - event.y) ** 2
      )
  ...
  async report() {
    ...
    const input = new ort.Tensor(
      ...
      Float32Array.from([
        ...
        this.SnChars + this.SnSpaces !== 0 ? this.SnErrorKeys / (this.SnChars + this.SnSpaces) : 0,
        this.SnRightClicked,
    ...
    const data = {
      ...
      CharactersRatio: this.SnKeyStrokes !== 0 ? this.SnChars / this.SnKeyStrokes : 0,
      ErrorKeyRatio:
        this.SnChars + this.SnSpaces !== 0 ? this.SnErrorKeys / (this.SnChars + this.SnSpaces) : 0,
      SnMouseDistance: this.SnMouseDistance,
      SnAppChange: this.SnAppChange,
      AppTimePerApp: this.AppTimePerApp,
      Stress: (output.variable as any).cpuData[0],
      Timestamp: Date.now()
    }
